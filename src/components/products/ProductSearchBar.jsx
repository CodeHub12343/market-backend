...
import { useProductSearchSuggestions, useProductSearchHistory, usePopularProductSearches, useSaveProductSearch } from '@/hooks/useProductSearch';
...
export default function ProductSearchBar({
  ...
  const { data: suggestionsData = [], isLoading: suggestionsLoading } = useProductSearchSuggestions(
    query,
    query.length >= 2
  );
  ...
  // Fetch history
  const { data: historyData = [], isLoading: historyLoading } = useProductSearchHistory(true);
  const history = Array.isArray(historyData) ? historyData : (historyData.history || []);

  // Fetch popular searches
  const { data: popularData = [], isLoading: popularLoading } = usePopularProductSearches(true);
  const popular = Array.isArray(popularData) ? popularData : (popularData.popular || []);

  // Save search callback
  const saveSearchMutation = useSaveProductSearch();
  ...
  const handleDeleteHistory = (e, historyId) => {
    e.stopPropagation();
    // TODO: Implement deleteProductSearchHistoryEntry hook
    ...
    <SearchContainer ref={searchInputRef} onFocus={() => setShowDropdown(true)}>
      ...
      <Dropdown ref={dropdownRef} isOpen={showDropdown}>
        ...
        {query.length >= 2 && !suggestionsLoading && suggestions.length === 0 && (
          <EmptyState>No products found for "{query}"</EmptyState>
        )}
        {!query && history.length === 0 && popular.length === 0 && (
          <EmptyState>Start typing to search for products</EmptyState>
        )}
      </Dropdown>
    </SearchContainer>
  );
